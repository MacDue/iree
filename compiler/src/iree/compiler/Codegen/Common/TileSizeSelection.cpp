...
SizesAndScalableFlags TilingConfig::getVectorTileSizes() {
  ...
  for (int i = 0; i < numDims; ++i) {
    unsigned hasValueCount = llvm::count_if(
        ArrayRef<bool>{parallelCommonSizes[i] || parallelCommonScalableFlags[i],
                       reductionSizes[i] || reductionScalableFlags[i],
                       parallelInnerSizes[i] || parallelInnerScalableFlags[i]},
        [](bool hasValue) { return hasValue; });
    assert(hasValueCount <= 1 &&
           "expected at most one non-zero/scalable tile size");
    (void)hasValueCount;
    vectorSizes[i] =
