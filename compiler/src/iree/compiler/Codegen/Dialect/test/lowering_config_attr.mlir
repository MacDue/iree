...
module {
  ...
  func.func @scalable_tile_sizes() attributes {
      lowering_config = #iree_codegen.lowering_config<tiling_levels = [[128, 128, 0], [1, [32], 0], [0, 0, 1], [0, 0, 0]]>} {
    return
  }
  // CHECK: #config = #iree_codegen.lowering_config<tiling_levels = {{\[}}[128, 128, 0], [1, [32], 0], [0, 0, 1], [0, 0, 0]{{\]}}>
}
...
module {
  ...
  func.func @scalable_tile_sizes() attributes {
      lowering_config = #iree_codegen.lowering_config<tiling_levels = [[128, 128, 0], {sizes=[1, [32], 0], interchange=[2, 1, 0]}, [0, 0, 1], [0, 0, 0]]>} {
    return
  }
  // CHECK: #config = #iree_codegen.lowering_config<tiling_levels = {{\[}}[128, 128, 0], {sizes = [1, [32], 0], interchange = [2, 1, 0]}, [0, 0, 1], [0, 0, 0]{{\]}}>
}
